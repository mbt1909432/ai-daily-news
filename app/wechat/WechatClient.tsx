...
interface WechatClientProps {
  newsData: DailyNews;
}

export default function WechatClient({ newsData }: WechatClientProps) {
  ...
  const badgeStyle: Record<Badge, { bg: string; color: string }> = {
    HOT: { bg: '#ef4444', color: '#fff' },
    NEW: { bg: '#00d4ff', color: '#000' },
    UPDATE: { bg: '#9333ea', color: '#fff' },
    IPO: { bg: '#f97316', color: '#fff' },
    LEGAL: { bg: '#2563eb', color: '#fff' },
    CHIP: { bg: '#059669', color: '#fff' },
    FUNDING: { bg: '#eab308', color: '#000' },
    POLICY: { bg: '#475569', color: '#fff' },
    EU: { bg: '#4f46e5', color: '#fff' },
    'OPEN SOURCE': { bg: '#16a34a', color: '#fff' },
  };
              ...
              {(Object.entries(newsByCategory) as [Category, typeof newsData.news][])
                ...
                .map(([cat, news]) => (
                  <section key={cat} style={{ marginBottom: '24px' }}>
                    ...
                    {news.map((n) => {
                      const badge = n.badge as Badge | undefined;
                      const style = badge ? badgeStyle[badge] : null;
                      return (
                        <div
                          key={n.id}
                          style={{
                        ...
                        >
                          ...
                          <h3
                            style={{
                              fontSize: '15px',
                              fontWeight: 600,
                              margin: badge ? '8px 0 8px 0' : '0 0 8px 0',
                              color: '#000',
                              lineHeight: 1.5,
                            }}
                          >
